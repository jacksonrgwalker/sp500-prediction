...
symbols = wiki_tables[0]

# Clean up the table
symbols.columns = symbols.columns.str.lower().str.replace(" ", "_", regex=False).str.replace("-", "_", regex=False)
...
old_symbols = wiki_tables[1]
old_symbols.columns = ['_'.join(set(c)) for c in old_symbols.columns.to_flat_index()]
...
removed = old_symbols[['removed_ticker', 'removed_security', 'date']].dropna()
added = old_symbols[['added_ticker', 'added_security', 'date']].dropna()
removed.columns = ['symbol', 'security', 'date_removed']
added.columns = ['symbol', 'security', 'date_added']

# if symbol has > 1 date_removed, keep the earliest date removed
removed = removed.sort_values(by=['date_removed']).drop_duplicates(subset=['symbol'], keep='first')

# if symbol has > 1 date_added, keep the earliest date added
added = added.sort_values(by=['date_added']).drop_duplicates(subset=['symbol'], keep='first')

old_symbols = added.merge(removed, on=['symbol'], how='outer', suffixes=('', '_removed'))
...
all_symbols = symbols.merge(old_symbols, on=['symbol'], how='outer', suffixes=('', '_old'))
